...
export function ProblemCard({ problem, isSelected, isProposed, onClick }: ProblemCardProps) {
  return (
    <div
      onClick={onClick}
      className={cn(
    ...
    >
      ...
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div className="flex items-center gap-2">
          ...
          <span className="inline-flex items-center gap-1 text-[11px] font-mono font-bold text-muted-foreground bg-neutral-100 dark:bg-neutral-800 px-2 py-0.5 rounded">
      ...
      {(problem.strategicTheme || problem.researchArea) && (
        <div className="grid grid-cols-2 gap-2 border-t border-border/50 pt-3 mt-1.5 text-xs text-muted-foreground">
          {problem.strategicTheme && (
            <div className="flex items-center gap-1.5 truncate" title={`Theme: ${problem.strategicTheme}`}>
          ...
          {problem.researchArea && (
            <div className="flex items-center gap-1.5 truncate" title={`Area: ${problem.researchArea}`}>
