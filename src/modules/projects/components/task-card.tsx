...
export function TaskCard({ task, onClick, onDragStart }: TaskCardProps) {
  ...
  return (
    <div
      ...
      onClick={onClick}
      className={cn(
        "group relative flex cursor-grab select-none flex-col gap-3 rounded-xl border border-transparent bg-surface p-3.5 shadow-sm transition-all duration-200 hover:-translate-y-0.5 hover:border-brand-secondary/30 hover:shadow-md active:cursor-grabbing"
      )}
    >
      ...
      <h4 className="m-0 line-clamp-2 text-sm font-extrabold leading-snug text-foreground transition-colors group-hover:text-brand-primary">
        {task.title}
      </h4>
