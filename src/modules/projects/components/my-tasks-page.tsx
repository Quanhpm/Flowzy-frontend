                ...
                <div className="grid gap-3 min-[761px]:hidden">
                  {myTasks.content.map((task) => (
                    <button
                      ...
                      className="grid min-h-11 min-w-0 gap-3 rounded-xl border border-border bg-surface p-4 text-left outline-none transition-[border-color,box-shadow] focus-visible:border-brand-secondary focus-visible:shadow-[0_0_0_4px_rgba(106,0,255,0.16)]"
                      key={task.id}
                      onClick={() => handleRowClick(Number(task.id), null)}
                      type="button"
                    >
